...
fn run_test(name: &str, near_end: &[f32], config: Config) -> Result<()> {
    ...
    let corr = if output.len() > skip {
        calculate_correlation(&near_end[skip..output.len()], &output[skip..])
    } else {
        calculate_correlation(&near_end, &output)
    };
    let in_rms = calculate_rms(&near_end[..output.len()]);
    let out_rms = calculate_rms(&output);

    println!("  {}: corr={:.4}, RMS {:.4}->{:.4} ({:+.1} dB)",
        name, corr, in_rms, out_rms,
        20.0 * (out_rms / in_rms.max(0.0001)).log10());
...
fn main() -> Result<()> {
    ...
    println!("\n=== AEC Config Tests (ref=silence) ===");

    // Baseline: No processing at all
    {
        let config = Config::default();
        run_test("No processing (baseline)", &near_end, config)?;
    }
    ...
    // Default AEC
    {
        ...
        run_test("AEC Full (default)", &near_end, config)?;
    }

    // AEC + high_pass_filter
    {
        let mut config = Config::default();
        config.echo_canceller = Some(EchoCanceller::Full { stream_delay_ms: None });
        config.high_pass_filter = Some(HighPassFilter { apply_in_full_band: true });
        run_test("AEC + HPF", &near_end, config)?;
    }

    // AEC delay=0ms
    {
        let mut config = Config::default();
        config.echo_canceller = Some(EchoCanceller::Full { stream_delay_ms: Some(0) });
        run_test("AEC delay=0ms", &near_end, config)?;
    }

    // AEC delay=8ms (match AEC's own estimate)
    {
        let mut config = Config::default();
        config.echo_canceller = Some(EchoCanceller::Full { stream_delay_ms: Some(8) });
        run_test("AEC delay=8ms", &near_end, config)?;
    }

    // AEC delay=16ms
    {
        let mut config = Config::default();
        config.echo_canceller = Some(EchoCanceller::Full { stream_delay_ms: Some(16) });
        run_test("AEC delay=16ms", &near_end, config)?;
    }

    // AEC delay=32ms
    {
        let mut config = Config::default();
        config.echo_canceller = Some(EchoCanceller::Full { stream_delay_ms: Some(32) });
        run_test("AEC delay=32ms", &near_end, config)?;
    }

    // AEC delay=64ms
    {
        let mut config = Config::default();
        config.echo_canceller = Some(EchoCanceller::Full { stream_delay_ms: Some(64) });
        run_test("AEC delay=64ms", &near_end, config)?;
    }
